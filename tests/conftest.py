...
it AS custom_components/house_cleaning), and the
directory on disk is `house-cleaning`, which is not an importable module
...
and symlinks to const.py and chores.py alone -- not the repo's own
__init__.py, which imports `homeassistant.config_entries` and would drag all
of core in behind it.

That is not a workaround, it is the enforcement of the rule chores.py's
docstring claims: this suite runs with Home Assistant ABSENT, so any
`homeassistant` import that creeps into const.py or chores.py fails the
...
PURE_MODULES = ("const.py", "chores.py")

_stage = Path(tempfile.mkdtemp(prefix="house_cleaning_test_"))
atexit.register(shutil.rmtree, _stage, True)

_pkg = _stage / "house_cleaning"
_pkg.mkdir()
